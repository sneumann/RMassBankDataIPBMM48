...
msmsXCMS <- msmsRead(msmsXCMS, filetable = ftable,
					readMethod = "xcms", mode = "pH", confirmMode = FALSE, useRtLimit = TRUE, 
					Args = Args, settings = getOption("RMassBank"), progressbar = "progressBarHook", MSe = FALSE)
setwd(oldpath)
msmsXCMS <- msmsWorkflow(msmsXCMS,steps=2:8, mode="pH", readMethod="xcms", findPeaksArgs = Args,
